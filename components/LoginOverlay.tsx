...
const LoginView: React.FC<any> = ({ onChangeView, onClose, onSwitchToAdmin, onSwitchToSignup, onLoginSuccess }) => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
      ...
      const storedUser = storedUsers.find((u: any) => u.email.toLowerCase() === email.toLowerCase() && u.password === password);

      if (isStandardDemo || isUnrestrictedDemo || storedUser) {
        if (rememberMe) {
        ...
        if (storedUser) loggedInRole = storedUser.role;
        else if (email === 'demo@parent.com') loggedInRole = 'parent';
        else if (email === 'demo@broker.com') loggedInRole = 'broker';
    ...
    return (
      ...
      >
        <div className="text-center space-y-2 mb-6">
          <motion.h2 className="text-3xl font-display font-bold text-gray-900 dark:text-white">{t('auth.welcome')}</motion.h2>
          <motion.p className="text-gray-500 dark:text-gray-400 text-sm">{t('auth.subtitle')}</motion.p>
        </div>
        ...
        <div className="flex p-1 bg-gray-100 dark:bg-black/40 rounded-xl relative">
          ...
          {(['self', 'parent', 'broker'] as const).map((r) => (
            ...
            >
               ...
               <span className="capitalize">{t(`auth.role.${r}`)}</span>
               {role === r && (
        ...
        <form className="space-y-4 pt-2" onSubmit={handleLogin}>
          ...
          <div className="flex justify-between items-center text-sm">
            <label className="flex items-center gap-2 cursor-pointer text-gray-500 dark:text-gray-400 select-none group">
              ...
              <span className={`text-xs transition-colors ${rememberMe ? 'text-purple-600 dark:text-purple-400 font-medium' : 'group-hover:text-gray-700 dark:group-hover:text-gray-200'}`}>{t('auth.remember')}</span>
            </label>
            <span className="text-red-500 text-xs h-4 px-2">{error}</span>
            <button type="button" onClick={() => onChangeView('forgot')} className="font-medium text-purple-600 dark:text-blue-400 hover:text-purple-800 dark:hover:text-blue-300 transition-colors">{t('auth.forgot')}</button>
          </div>
          ...
          >
            <span className="relative flex items-center justify-center gap-2">
              {isLoading ? t('common.loading') : <>{t('auth.signin')} <ArrowRight size={18} /></>}
            </span>
          </motion.button>
        </form>
        ...
        <div className="text-center mt-6 space-y-4">
            ...
            {t('auth.noAccount')} {' '}
            <button onClick={() => onSwitchToSignup ? onSwitchToSignup() : onChangeView('register')} className="font-bold text-purple-600 dark:text-blue-400 hover:underline">{t('auth.register')}</button>
            </p>
            {onSwitchToAdmin && (
...
const RegisterView: React.FC<{ 
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    const newUser = {
      ...
      religion: '', caste: '', age: 0, gender: '', location: ''
    };
  ...
  return (
    ...
    >
       <div className="text-center space-y-2 mb-4">
        <h2 className="text-3xl font-display font-bold text-gray-900 dark:text-white">{t('auth.register.title')}</h2>
      ...
      <div className="flex p-1 bg-gray-100 dark:bg-black/40 rounded-xl">
         <button onClick={() => setActiveTab('self')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'self' ? 'bg-white dark:bg-gray-800 text-purple-600 shadow' : 'text-gray-500'}`}>{t('auth.role.self')}</button>
         <button onClick={() => setActiveTab('parent')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'parent' ? 'bg-white dark:bg-gray-800 text-purple-600 shadow' : 'text-gray-500'}`}>{t('auth.role.parent')}</button>
      </div>
      ...
      {activeTab === 'parent' ? (
         <div className="py-8 text-center space-y-4">
            ...
            >
               {t('auth.parent')} <ArrowRight size={16} className="inline ml-2" />
            </motion.button>
         </div>
      ) : (
         <form className="space-y-3" onSubmit={handleRegister}>
            ...
            >
               ...
               <span className="relative">{isLoading ? t('common.loading') : t('auth.submit')}</span>
            </motion.button>
         </form>
      )}
...
const ForgotView: React.FC<any> = ({ onChangeView }) => {
  ...
  >
    ...
    <div className="space-y-2 mb-8">
      <h2 className="text-2xl font-display font-bold text-gray-900 dark:text-white">{t('auth.forgot')}</h2>
    ...
    <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
      <AnimatedInput icon={<Mail size={20} />} type="email" label={t('auth.email')} placeholder="john@example.com" />
